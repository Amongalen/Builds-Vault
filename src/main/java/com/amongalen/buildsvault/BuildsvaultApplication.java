...
@AllArgsConstructor
@Slf4j
public class BuildsvaultApplication implements CommandLineRunner {

    private final BuildGuideRepository repository;
    ...
    public void run(String... args) throws Exception {
        ...
        BuildGuide guide = new BuildGuide("test Build", createPoB());
        repository.save(guide);
        BuildGuide guide2 = new BuildGuide("test Build", createPoB());
        repository.save(guide2);
        BuildGuide guide3 = new BuildGuide("test Build2", createPoB());
        repository.save(guide3);

        log.info("All found builds:");
        List<BuildGuide> all = repository.findAll();
        for (BuildGuide build : all) {
            log.info("name={}, id={}", build.getName(), build.getId());
        }

        log.info("------------------------------");
        log.info("All found builds by name \"test Build\":");
        List<BuildGuide> allByName = repository.findAllByName("test Build");
        for (BuildGuide build : allByName) {
            log.info("name={}, id={}", build.getName(), build.getId());
        }
        log.info("------------------------------");
        log.info("All found by id 1:");
        Optional<BuildGuide> guideByIdOptional = repository.findById("1");
        if (guideByIdOptional.isPresent()) {
            BuildGuide guideById = guideByIdOptional.get();
            log.info("name={}, id={}", guideById.getName(), guideById.getId());
        }
    }

    private static PathOfBuilding createPoB() {
        ...
        PoBImporter pobImporter = new PoBImporterImpl();
